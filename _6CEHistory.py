...
from tesseract import tesseract_ as tesseract
...
def verifyInput(returnValue, regexMatch):
    while True:
        if re.search(regexMatch, f"^{returnValue}$"):
            break
...
def doClaims():
    ...
    pressKeyList(["ctrl+o", "-3,assertTopWindow,Open", "ctrl+c", "esc"])
    ...
    secondClaimNumber = tesseract()
    print(f"Second: {secondClaimNumber}")
