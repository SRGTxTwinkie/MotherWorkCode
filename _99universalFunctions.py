...
def openClaim(hotkey="f13", reprocess=False, click=ClickOptions.Default, f3=True, suffix=1, moveToWindow=True, highlight=True):
    ...
    while True:
        try:
            ...
            print("There was an error, copy the claim number and press enter")
...
def getCorrectWindow(isFacility:str, op:bool=False):
    ...
    if not op:
        ...
        y = "Medical" if not isFacility else "Hospital"
        while True:
            windowText = win32gui.GetWindowText(win32gui.GetForegroundWindow())
            regex = re.search(f"{y}.*Unassigned", windowText)
            if regex:
                break
            pressKey("ctrl+shift+tab", delay=0.2)
            sleep(0.5)
