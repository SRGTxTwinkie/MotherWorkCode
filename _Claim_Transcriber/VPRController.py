...
class VPRController:
    def __init__(self, old=False) -> None:
        ...
        try:
            if not old:
                ...
                self.reference = self.applicationReference.connect(
                    title_re="Viewer Printer"
                )
                self.vpReference = self.reference["Viewer Printer"]
        except:
            raise Exception(
                "Could not connect to Viewer Printer...\nMaybe not open?\nError Dialouge open?"
            )
    ...
    def updateClaimNum(
        ...
        self.vpReference.Edit0.set_edit_text(claimNum)
        if search:
            self.vpReference.SearchButton.click()
        if log:
            print("Claim Number Updated")
